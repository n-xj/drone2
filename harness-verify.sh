...
# ── 1. Cursor Rules（5个）──────────────────────────────
echo -e "${YELLOW}[1/7] Cursor Rules 文件体系${NC}"
...
# ── 2. Hooks───────────────────────────────────────────
echo -e "${YELLOW}[2/7] Cursor Hooks 自动化体系${NC}"
check_file ".cursor/hooks.json"
check_file ".cursor/hooks/harness-reminder.ps1"
echo ""
...
if [ $ERRORS -eq 0 ]; then
  ...
  echo -e "${GREEN}   Cursor Rules 文件数量：$RULES_COUNT（均包含 alwaysApply: true）${NC}"
  echo ""
  echo "Harness 模板已就绪，可直接使用。"
  echo "新建 Spring Boot 子项目后，在 Cursor 中打开工作区根目录即可开始受监管的开发。"
else
